...
function Brick:hit()
    ...
    --local shouldSpawnPower = math.random(1, 10)
    --local shouldSpawnPower = 1

    --if shouldSpawnPower == 1 then
        --table.insert(PlayState.powerups, Powerup(self.x, self.y))
    --end
...
function Brick:render()
    if self.inPlay then
        if self.locked == false then
            ...
            love.graphics.draw(gTextures['main'], gFrames['lockedblock'], self.x, self.y)
        end
    end
end
