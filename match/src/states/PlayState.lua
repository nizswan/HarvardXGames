...
function PlayState:enter(params)
    ...
    if self.level == 1 then
        self.board = Board(VIRTUAL_WIDTH - 272, 16, true)
    else
        self.board = Board(VIRTUAL_WIDTH - 272, 16, false)
    end
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) == 1 then
                -- Attempt to swap and calculate matches
                self:trySwapTiles(self.highlightedTile, self.board.tiles[y][x])
            else
                self.highlightedTile = nil
                gSounds['error']:play()
            end
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for m, tile in pairs(match) do
                if tile.shiny then
                    -- Remove the entire row and update score and timer for each tile removed
                    local tilesRemoved = self.board:removeRow(tile.gridY)
                    self.score = self.score + tilesRemoved * 50
                    --self.timer = self.timer + tilesRemoved
                    --this addsd too much time for each shiny swapped
                end
            end
        end
